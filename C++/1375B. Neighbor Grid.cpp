...
// Date   : 03.07.2022 09:44:02:80 PM +06:00 Asia/Dhaka
...
void solve() {
	...
	rep(i, 0, n) {
		rep(j, 0, m) {
			ll neighbors = 0;
			if (i - 1 >= 0) neighbors += 1;
			if (j - 1 >= 0) neighbors += 1;
			if (i + 1 < n ) neighbors += 1;
			if (j + 1 < m) neighbors += 1;
			if (arr[i][j] > neighbors) {ok = 0; break;}
			arr[i][j] = neighbors;
		}
	}
	if (ok) {
