...
void solve() {
	...
	ll ans = 0;
	for (auto itr = ++leftR.rbegin(); itr != leftR.rend(); itr++) {
	   // cout<<itr->ff<<ta<<rightL[itr->ff]<<ta<<itr->ss<<endl;
		if (rightL[itr->ff] != 0 && rightL[itr->ff]>itr->ss) {
		  //  cout<<itr->ff<<endl;
			ans = max(n-rightL[itr->ff] + itr->ss+1, ans);
			break;
		}
		// cout << rightL[itr->ff] << ta << itr->ff << ta << itr->ss << endl;
	}
	cout << ans << endl;
}
