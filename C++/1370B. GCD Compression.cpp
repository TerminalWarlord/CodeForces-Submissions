...
void solve() {
	...
	ll count = 0;
	ll oddTraverse = sz(odd);
	if (oddTraverse & 1) oddTraverse--;
	ll evenTraverse = sz(even);
	if (evenTraverse & 1) evenTraverse--;
	for (int i = 0; i < oddTraverse; i += 2) {
		if (count + 2 > n) break;
		cout << odd[i] << ta << odd[i + 1] << endl;
		count += 1;
	}
	for (int i = 0; i < evenTraverse; i += 2) {
		if (count + 2 > n) break;
		cout << even[i] << ta << even[i + 1] << endl;
		count += 1;
