...
// Date   : 25.06.2022 01:09:46:58 PM +06:00 Asia/Dhaka
...
void solve() {
	...
	cin >> n >> m;
	vector<string> arr(n);
	unordered_map<string, ll> mpp, samePalmap;
	string samePal = "";
	rep(i, 0, n) {
		cin >> arr[i];
		int ok = 1;
	...
	string ansBackward = "";
	for (auto it : arr) {
		if (it == samePal || samePalmap[it]) continue;
		string tmp = it, tmp2 = tmp;
		reverse(all(tmp2));
		debug(tmp);
		if (tmp == tmp2) {samePal = tmp; continue;}
		if (mpp[tmp2] && tmp != tmp2) {
			ans += tmp;
			ansBackward = tmp2 + ansBackward;
		}
		samePalmap[tmp] = 1;
		samePalmap[tmp2] = 1;
	}
	...
	debug(ansBackward);
	debug(samePal);
	cout << sz(ans) + sz(samePal) + sz(ansBackward) << endl;
	cout << ans << samePal << ansBackward << endl;
}
int main()
{
	init_code();
