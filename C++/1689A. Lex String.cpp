...
// Date   : 11.06.2022 12:14:18:05 AM +06:00 Asia/Dhaka
...
void solve() {
	...
	ll ind1 = 0;
	ll ind2 = 0;
	while (ind1 < sz(a) && ind2 < sz(b)) {
		ll tmp = k;
		while (tmp > 0 && a[ind1] <= b[ind2] && ind1 < sz(a) && ind2 < sz(b)) {
			ans.pb(a[ind1]);
			tmp -= 1;
			ind1 += 1;
		}
		if (tmp == 0 && a[ind1] < b[ind2] && ind1 < sz(a) && ind2 < sz(b)) {
			ans.pb(b[ind2]);
			ind2 += 1;
		}
		tmp = k;
