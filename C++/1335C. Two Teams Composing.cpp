...
// Date   : 02.07.2022 10:55:17:88 PM +06:00 Asia/Dhaka
...
void solve() {
	...
	for (auto &it : arr) {
		cin >> it;
		if (mpp[it] == 0) mpp[it] += 1;
		else mpp[it] += 1;
		same = max(same, mpp[it]);
	}
	if (same == sz(mpp)) cout << same - 1 << endl;
	else cout << min(sz(mpp) * 1ll, same) << endl;
}
