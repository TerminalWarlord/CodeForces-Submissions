...
// Date   : 16.06.2022 12:26:15:69 AM +06:00 Asia/Dhaka
...
void solve() {
	ll n;
	cin >> n;
	vector<ll> arr(n), newArr, freq(10, 0);
	rep(i, 0, n) {
		cin >> arr[i];
		arr[i] %= 10;
		if (freq[arr[i]] == 0) newArr.pb(arr[i]);
		freq[arr[i]] += 1;
	}
	debug(freq);
	debug(newArr);
	...
	if (freq[1] >= 3) {
		...
		rep(i, 0, sz(newArr)) {
			if (freq[newArr[i]] < 2) continue;
			rep(j, 0, sz(newArr)) {
				if (i == j) continue;
				ll tmp = newArr[i] * 2 + newArr[j];
				if (tmp % 10 == 3) {
					ok = 1;
					break;
				}
			...
			rep(i, 0, sz(newArr)) {
				rep(j, 0, sz(newArr)) {
					rep(k, 0, sz(newArr)) {
						...
						ll tmp = newArr[i] + newArr[j] + newArr[k];
						if (tmp % 10 == 3) {
							ok = 1;
							break;
						}
