...
// Date   : 21.06.2022 06:40:22:45 PM +06:00 Asia/Dhaka
...
void solve() {
	...
	rep(i, 0, n) {
		...
		if (arr[i] & 1) odd += 1, oddMax = max(arr[i], oddMax);
		else even += 1;
		if (arr[i] & 1) sum += arr[i] + 1;
		else sum += arr[i];
	}
	debug(odd); debug(oddMax);
	if ((oddMax < 3 && (odd > 1 && even == 0)) || (even == 0 && odd == 1)) cout << -1 << endl;
	else cout << (ll)ceil(sum / 2.00) << endl;
}
