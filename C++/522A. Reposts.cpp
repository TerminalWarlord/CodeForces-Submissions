...
// Date   : 17.06.2022 02:23:34:02 PM +06:00 Asia/Dhaka
...
ll dfs(string node, map<string, ll> &vis, map<string, vector<string>> &adj) {
	vis[node] = 1;
	ll ans = 0;
	for (auto i : adj[node]) {
		if (!vis[i]) {
			ans = max(dfs(i, vis, adj), ans);
		}
	}
	return ans + 1;
...
void solve() {
	...
	rep(i, 0, n) {
		string a, b, c;
		cin >> a >> b >> c;
		rep(i, 0, sz(a)) a[i] = tolower(a[i]);
		rep(i, 0, sz(c)) c[i] = tolower(c[i]);
		adj[a].pb(c);
		adj[c].pb(a);
	}
	cout << dfs("polycarp", vis, adj) << endl;
}
