...
void solve() {
	ll n;
	cin >> n;
	vector<ll> a(n), b(n), aClone(n), bClone(n);
	for (auto &x : a) cin >> x;
	for (auto &x : b) cin >> x;
	aClone = a;
	bClone = b;
	...
	rep(i, 0, n) {
		if (a[i] == aClone[i] && b[i] == bClone[i]) continue;
		rep(j, i + 1, n) {
			if (a[j] == aClone[i] && b[j] == bClone[i]) {
				res.pb({i + 1, j + 1});
				swap(a[j], a[i]);
				swap(b[j], b[i]);
			}
		}
		if (a[i] != aClone[i] || b[i] != bClone[i]) {
			ok = 0;
			break;
		}
	}
