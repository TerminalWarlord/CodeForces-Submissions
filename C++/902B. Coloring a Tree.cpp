...
// Date   : 17.06.2022 06:31:40:50 PM +06:00 Asia/Dhaka
...
void dfs(ll node, ll par,  vector<ll> adj[], vector<ll> &vis, vector<ll> &c, ll &ans) {
	// vis[node] = 1;
	if (node != 1 && c[node] != c[par]) ans += 1;
	for (auto i : adj[node]) {
		if (i == par) continue;
		// if (!vis[i]) {
		dfs(i, node, adj, vis, c, ans);
		// }
	}
	// return ans;
