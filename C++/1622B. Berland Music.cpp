...
void solve() {
	ll n;
	cin >> n;
	vector<ll> arr(n), arr2(n), ones, zeroes;
	...
	cin >> s;
	rep (i, 0, n) {
		if (s[i] == '0') zeroes.pb(arr[i]);
		else ones.pb(arr[i]);
	}
	sort(all(arr), greater<int>());
	sort(all(ones), greater<int>());
	sort(all(zeroes), greater<int>());
	rep(i, 0, sz(ones)) arr2[mp[ones[i]]] = arr[i];
	ll j = sz(ones);
	rep(i, 0, sz(zeroes)) arr2[mp[zeroes[i]]] = arr[j], j++;
	for (auto it : arr2) cout << it << ta;
	cout << endl;
}
