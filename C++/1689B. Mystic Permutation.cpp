...
// Date   : 11.06.2022 12:59:02:42 AM +06:00 Asia/Dhaka
...
void solve() {
	...
	rep(i, 0, n) {
		if (tmp[i] != arr[i]) {
			...
			ll ind = i;
			while (ind < n && tmp[i] == tmp[ind]) {
				ind += 1;
			}
			if (ind < n && tmp[i] != tmp[ind]) {
				swap(tmp[i], tmp[ind]);
			}
			else {
				ind = i;
				while (ind >= 0 && tmp[i] == tmp[ind]) {
					ind -= 1;
				}
				if (ind >= 0 && tmp[i] != tmp[ind]) {
					swap(tmp[i], tmp[ind]);
				}
			}
		}
