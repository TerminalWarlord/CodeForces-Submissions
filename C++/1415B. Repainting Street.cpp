...
// ll maxiByValue(unordered_map<ll, ll> &mpp) {
// 	ll maxi = -1e18;
// 	ll maxiVal = 1;
// 	for (auto it : mpp) {
// 		if (it.ss > maxi) {
...
void solve() {
	...
	// ll maxi = maxiByValue(mpp);
	// if (arr[0] != maxi && arr[n - 1] != maxi && mpp[maxi] == m) {
	// 	cout << 1 << endl;
	// 	return;
	// }
	ll count;
	...
	// ll prevI = 0;
	// rep(i, 0, n) {
	// 	// if (arr[i - 1] != maxi || arr[i] != maxi) count++;
	// 	if (i > 0 && arr[i - 1] == arr[i] && arr[i] == maxi) continue;
	// 	if (arr[i] == maxi) {
	// 		//  cout<<i <<ta<< prevI<<endl;
	// 		count += i - prevI;
	// 		prevI = i;
