...
// Date   : 30.06.2022 12:27:23:57 PM +06:00 Asia/Dhaka
...
void solve() {
	...
	while (k) {
		...
		if (arr2Valid) {
			...
			if (k == 1) tmp = n - sum2;
			sum2 += tmp;
			if (tmp & 1 || tmp <= 0) arr2Valid = 0;
			arr2.pb(tmp);
		}
		if (!arrValid && !arr2Valid) break;
		k -= 1;

	}
	if (arrValid || arr2Valid) {
		if (arr2Valid) arr = arr2;
		cout << "YES" << endl;
		for (auto it : arr) cout << it << ta;
		cout << endl;
	}
	else cout << "NO" << endl;
