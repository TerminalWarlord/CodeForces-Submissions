...
void solve() {
	...
	if (sz(s) & 1) {
		...
		if ((int)s[0] > (int)s[sz(s) - 1]) {
			bobScore += (int)s[sz(s) - 1] - 96;
		}
		else {
			bobScore += (int)s[0] - 96;
		}
		aliceScore = total - bobScore;

	}
	else {
		rep(i, 0, sz(s)) {
			aliceScore += (int)s[i] - 96;
		}
	}
	// cout << aliceScore << ta << bobScore << endl;
	if (aliceScore >= bobScore) cout << "Alice" << ta << abs(aliceScore - bobScore) << endl;
	else cout << "Bob" << ta << abs(aliceScore - bobScore) << endl;
}
