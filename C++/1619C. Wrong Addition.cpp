...
// Date   : 21.06.2022 02:00:31:38 PM +06:00 Asia/Dhaka
...
void solve() {
	string s, t;
	cin >> s >> t;
	ll cnt1 = sz(s) - 1, cnt2 = sz(t) - 1;
	ll res = 0, sCount = 0;
	while (cnt2 >= 0) {
		if (cnt1 >= 0 && (int)s[cnt1] > (int)t[cnt2]) {
			string tmp;
			if (t[cnt2 - 1] != '1') {cout << -1 << endl; return;}
			tmp.pb(t[cnt2 - 1]);
			tmp.pb(t[cnt2]);
			ll n1 = stoi(tmp);
			n1 -= s[cnt1] - 48;
			res += n1  *  (ll)pow(10, sCount);
			cnt2 -= 1;
		}
		else {
			ll n1 = t[cnt2];
